...
- 요약: LLM — 기본 **Claude API**, 설정으로 **OpenAI 호환 API** 또는 **로컬 LLM(Ollama)** 전환 가능
...
|---|---|---|---|
| `claude` (기본) | `llm.claude.model` | `ANTHROPIC_API_KEY` | `pip install anthropic` |
| `openai_compat` | `llm.openai_compat.model`, `.base_url` | `OPENAI_API_KEY` | `pip install openai`. OpenAI/Together/Groq/vLLM 등 OpenAI 호환 엔드포인트 모두 가능 |
...
1. **새 저장소 생성** 후 이 폴더 내용을 push
   ...
   git init
   git add .
   git commit -m "init: weekly paper digest"
   git branch -M main
...
5. **첫 실행**: Actions 탭 → "Weekly Paper Digest" → *Run workflow* (원하면 `dry_run`으로 먼저 점검). 이후 매주 일요일 23:00 UTC(= 월요일 08:00 KST)에 자동 실행됩니다.
